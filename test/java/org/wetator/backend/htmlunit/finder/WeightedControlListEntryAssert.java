...
public abstract class WeightedControlListEntryAssert {
  ...
  public static void assertEntriesSorted(final SortedEntryExpectation anExpected, final WeightedControlList anActual) {
    Assert.assertEquals(anExpected == null ? "" : anExpected.toString(),
        toSortedEntryExpectationString(anActual.getEntriesSorted()));
  }

  private static String toSortedEntryExpectationString(final List<WeightedControlList.Entry> anEntryList) {
    ...
    for (WeightedControlList.Entry tmpEntry : anEntryList) {
      ...
      tmpDescribingText = tmpDescribingText.substring(0, tmpDescribingText.indexOf(' '))
          + tmpDescribingText.substring(tmpDescribingText.lastIndexOf(' '), tmpDescribingText.length());
      tmpEntryList.append(tmpDescribingText);
  ...
  public static class SortedEntryExpectation {
    ...
    public String toString() {
      ...
      for (ExpectedControl tmpEntry : entries) {
        tmpEntryList.append(tmpEntry.element.getSimpleName() + " (id='" + tmpEntry.id + "')");
