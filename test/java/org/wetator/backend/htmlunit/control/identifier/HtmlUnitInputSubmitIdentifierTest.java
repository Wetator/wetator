...
public class HtmlUnitInputSubmitIdentifierTest extends AbstractHtmlUnitControlIdentifierTest {
  ...
  public void byValue() throws IOException, InvalidInputException {
    ...
    Assert.assertEquals(
        "[HtmlSubmitInput 'ClickMe' (id='myId')] found by: BY_LABELING_TEXT coverage: 0 distance: 0 start: 0 index: 5",
        tmpFound.getEntriesSorted().get(0).toString());
  }
  ...
  public void byTableCoordinates() throws IOException, InvalidInputException {
    ...
    Assert.assertEquals(
        "[HtmlSubmitInput 'ClickMe' (id='myId_2_3')] found by: BY_TABLE_COORDINATE coverage: 0 distance: 62 start: 62 index: 45",
        tmpFound.getEntriesSorted().get(0).toString());
  }
}
