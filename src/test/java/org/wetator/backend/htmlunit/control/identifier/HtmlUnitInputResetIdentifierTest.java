...
public class HtmlUnitInputResetIdentifierTest extends AbstractHtmlUnitControlIdentifierTest {
  ...
  public void byDataTestid() throws IOException, InvalidInputException {
    ...
    final String tmpHtmlCode = "<html><body>"
        + "<form action='test'>"
        + "<input id='myId' type='reset' value='ClickMe' data-testid='myDataTestId'>"
        + "</form>"
        + "</body></html>";
    // @formatter:on
  ...
  public void inTable() throws IOException, InvalidInputException {
    ...
    assertEquals(1, tmpEntriesSorted.size());
    assertEquals(
        "[HtmlResetInput 'ClickMe' (id='myId_2_3')] found by: BY_LABEL deviation: 0 distance: 62 start: 62 hierarchy: 0>1>3>5>22>36>44>45 index: 45",
        tmpEntriesSorted.get(0).toString());
  }
}
