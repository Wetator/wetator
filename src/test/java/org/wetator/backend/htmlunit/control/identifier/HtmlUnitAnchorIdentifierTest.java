...
public class HtmlUnitAnchorIdentifierTest extends AbstractHtmlUnitControlIdentifierTest {
  ...
  public void byDataTestid() throws IOException, InvalidInputException {
    ...
    final String tmpHtmlCode = "<html><body>"
        + "<form action='test'>"
        + "<a id='myId' name='myName' data-testid='myDataTestId' href='snoopy.php'>"
        + "<p>AnchorWithText</p>"
        + "</a>"
        + "</form>"
        + "</body></html>";
    ...
    assertEquals(1, tmpEntriesSorted.size());
    assertEquals(
        "[HtmlAnchor 'AnchorWithText' (id='myId') (name='myName') (data-testid='myDataTestId')] found by: BY_DATE_TESTID deviation: 0 distance: 0 start: 0 hierarchy: 0>1>3>4>5 index: 5",
        tmpEntriesSorted.get(0).toString());
  }
  ...
  public void inTable() throws IOException, InvalidInputException {
    ...
    assertEquals(1, tmpEntriesSorted.size());
    assertEquals(
        "[HtmlAnchor 'ClickMe' (id='myId_2_3')] found by: BY_LABEL deviation: 0 distance: 62 start: 62 hierarchy: 0>1>3>5>22>38>47>48 index: 48",
        tmpEntriesSorted.get(0).toString());
  }
}
