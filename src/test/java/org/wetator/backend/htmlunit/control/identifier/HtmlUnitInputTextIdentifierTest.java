...
public class HtmlUnitInputTextIdentifierTest extends AbstractHtmlUnitControlIdentifierTest {
  ...
  public void byDataTestid() throws IOException, InvalidInputException {
    ...
    final String tmpHtmlCode = "<html><body>"
        + "<form action='test'>"
        + "<input id='myId' name='myName' type='text' data-testid='myDataTestId'>"
        + "</form>"
        + "</body></html>";
    // @formatter:on
    ...
    assertEquals(1, tmpEntriesSorted.size());
    assertEquals(
        "[HtmlTextInput (id='myId') (name='myName') (data-testid='myDataTestId')] found by: BY_DATE_TESTID deviation: 0 distance: 0 start: 0 hierarchy: 0>1>3>4>5 index: 5",
        tmpEntriesSorted.get(0).toString());
  }
