...
public class HtmlUnitButtonIdentifierTest extends AbstractHtmlUnitControlIdentifierTest {
  ...
  public void byDataTestid() throws IOException, InvalidInputException {
    ...
    final String tmpHtmlCode = "<html><body>"
        + "<form action='test'>"
        + "<button id='myId' name='myName' data-testid='myDataTestId'>"
        + "<p>ButtonWithText</p>"
        + "</button>"
        + "</form>"
        + "</body></html>";
    ...
    assertEquals(1, tmpEntriesSorted.size());
    assertEquals(
        "[HtmlButton 'ButtonWithText' (id='myId') (name='myName') (data-testid='myDataTestId')] found by: BY_DATE_TESTID deviation: 0 distance: 0 start: 0 hierarchy: 0>1>3>4>5 index: 5",
        tmpEntriesSorted.get(0).toString());
  }
