 ...
 * @author rbri
 */
public class HtmlUnitInputEmailIdentifier extends AbstractMatcherBasedIdentifier {

  @Override
  public boolean isHtmlElementSupported(final HtmlElement aHtmlElement) {
    return aHtmlElement instanceof HtmlEmailInput || aHtmlElement instanceof HtmlLabel
        && ((HtmlLabel) aHtmlElement).getLabeledElement() instanceof HtmlEmailInput;
  }
  ...
  protected void addMatchers(final WPath aWPath, final HtmlElement aHtmlElement,
    ...
    if (aWPath.getLastNode() != null) {
      ...
      if (aHtmlElement instanceof HtmlEmailInput) {
        // element specific
      ...
      aMatchers.add(new ByTableCoordinatesMatcher(htmlPageIndex, tmpPathSearchPattern, tmpPathSpot,
          aWPath.getTableCoordinatesReversed(), HtmlEmailInput.class));
    }
  }

  @Override
  protected IControl createControl(final MatchResult aMatch) {
    return new HtmlUnitInputEmail((HtmlEmailInput) aMatch.getHtmlElement());
  }
}