...
/**
 * This is the implementation of the HTML element 'input Email' (&lt;input type="Email"&gt;) using HtmlUnit as
 * backend.
 *
 * @author rbri
 * @author frank.danek
 */
@ForHtmlElement(HtmlEmailInput.class)
@IdentifiedBy(HtmlUnitInputEmailIdentifier.class)
public class HtmlUnitInputEmail extends HtmlUnitBaseControl<HtmlEmailInput>
    implements ISettable, IHtmlUnitDisableable<HtmlEmailInput>, IHtmlUnitFocusable<HtmlEmailInput> {

  /**
   * The constructor.
   *
   * @param anHtmlElement the {@link HtmlEmailInput} from the backend
   */
  public HtmlUnitInputEmail(final HtmlEmailInput anHtmlElement) {
    super(anHtmlElement);
  }

  @Override
  public String getDescribingText() {
    return HtmlElementUtil.getDescribingTextForHtmlEmailInput(getHtmlElement());
  }

  @Override
  public void setValue(final WetatorContext aWetatorContext, final SecretString aValue, final File aDirectory)
      throws ActionException {
    final HtmlEmailInput tmpHtmlEmailInput = getHtmlElement();

    if (tmpHtmlEmailInput.isDisabled()) {
      final String tmpMessage = Messages.getMessage("elementDisabled", getDescribingText());
      throw new ActionException(tmpMessage);
    }
    if (tmpHtmlEmailInput.isReadOnly()) {
      final String tmpMessage = Messages.getMessage("elementReadOnly", getDescribingText());
      throw new ActionException(tmpMessage);
    }

    try {
      final HtmlPage tmpHtmlPage = (HtmlPage) tmpHtmlEmailInput.getPage();
      DomElement tmpFocusedElement = tmpHtmlPage.getFocusedElement();
      if (tmpFocusedElement == null || tmpHtmlEmailInput != tmpFocusedElement) {
        tmpHtmlEmailInput.click();
        ...
        if (tmpHtmlEmailInput != tmpFocusedElement) {
  ...
  public boolean isDisabled(final WetatorContext aWetatorContext) {
    ...
    return tmpHtmlEmailInput.isDisabled() || tmpHtmlEmailInput.isReadOnly();
  }
}
