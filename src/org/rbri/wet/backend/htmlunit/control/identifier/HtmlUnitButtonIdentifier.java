...
public class HtmlUnitButtonIdentifier extends AbstractHtmlUnitControlIdentifier {
  ...
  public WeightedControlList identify(WPath aWPath, HtmlElement aHtmlElement) {
    ...
    tmpMatches.addAll(new ByInnerImageMatcher(htmlPageIndex, tmpPathSearchPattern, tmpPathSpot, tmpSearchPattern)
        .matches(aHtmlElement));

    tmpMatches.addAll(new ByTextMatcher(htmlPageIndex, tmpPathSearchPattern, tmpPathSpot, tmpSearchPattern)
        .matches(aHtmlElement));
    ...
    for (MatchResult tmpMatch : tmpMatches) {
      tmpResult.add(new HtmlUnitButton((HtmlButton) tmpMatch.getHtmlElement()), tmpMatch.getFoundType(),
